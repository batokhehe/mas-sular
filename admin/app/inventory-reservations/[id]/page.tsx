    ...
    <AdminShell requiredPermissions={ROUTE_PERMISSIONS.inventoryReservations}>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Penahanan stok dan riwayat siklusnya.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-red-100 bg-red-50 p-6 text-sm text-red-700">Gagal memuat reservasi ini.</div>
      ) : (
        <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          <Card>
            ...
            <div className="mt-4 grid gap-4 text-sm text-gray-700 sm:grid-cols-2">
              ...
              <div>
                <p className="text-xs uppercase text-gray-400">Jumlah</p>
                <p className="mt-2 text-gray-700">Direservasi {data.reservedQty} · Terkonfirmasi {data.committedQty} · Dilepas {data.releasedQty}</p>
              </div>
              <div>
                <p className="text-xs uppercase text-gray-400">Kedaluwarsa</p>
                <p className="mt-2 text-gray-700">{data.expiresAt ? new Date(data.expiresAt).toLocaleString('id-ID') : 'Tanpa batas waktu'}</p>
              </div>
            </div>
          </Card>
          ...
          <Card>
            ...
            <ol className="mt-4 space-y-0">
              {(data.history ?? []).length === 0 ? (
                <p className="text-sm text-gray-500">Tidak ada riwayat tercatat.</p>
              ) : (
                (data.history ?? []).map((h, index, arr) => (
                  <li key={h.id} className="flex gap-3">
                    ...
                    <div className="pb-4">
                      <p className="text-sm font-medium text-gray-900">{reservationStatusLabel(h.status)}</p>
