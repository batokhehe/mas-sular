  ...
  return (
    <AdminShell requiredPermissions={ROUTE_PERMISSIONS.systemLogs}>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Detail Request</h2>
          <p className="mt-1 flex items-center gap-2 font-mono text-sm text-gray-500">
            {requestId}
            <button onClick={() => requestId && copy(requestId)} title="Salin Request ID" className="text-gray-300 hover:text-gray-600"><Copy className="h-4 w-4" /></button>
          </p>
        </div>
        <Link href="/system/requests" className="text-sm font-medium text-[#465fff] hover:underline">← Kembali ke Penjelajah Request</Link>
      </div>
      ...
      {query.isLoading ? (
        ...
        <Card>
          <div className="flex flex-col items-center gap-3 p-10 text-center">
            ...
            <p className="text-sm text-red-600">Gagal memuat request ini (mungkin sudah dihapus oleh kebijakan retensi).</p>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
            ...
            <Card>
              <p className="text-xs uppercase text-gray-400">Durasi</p>
            ...
            <Card>
              <p className="text-xs uppercase text-gray-400">Pengguna</p>
            ...
            <Card>
              <p className="text-xs uppercase text-gray-400">IP · Klien</p>
            ...
            <Card>
              <p className="text-xs uppercase text-gray-400">Log</p>
              <p className="mt-1 text-2xl font-semibold text-gray-900">{d.summary.totalLogs}</p>
              <p className="mt-1 text-xs">
                <span className={d.summary.errorCount > 0 ? 'font-semibold text-red-600' : 'text-gray-400'}>{d.summary.errorCount} error</span>
                <span className="text-gray-300"> · </span>
                <span className={d.summary.warningCount > 0 ? 'font-semibold text-amber-600' : 'text-gray-400'}>{d.summary.warningCount} peringatan</span>
              </p>
            </Card>
          </div>
          ...
          {(d.related.orderId || d.related.paymentId || d.related.shipmentId || d.related.userId) ? (
            <Card>
              <CardTitle>Terkait</CardTitle>
              <div className="mt-3 flex flex-wrap gap-2">
                {d.related.orderId ? <RelatedLink href={`/orders/${d.related.orderId}`} label="Pesanan" id={d.related.orderId} /> : null}
                {d.related.paymentId ? <RelatedLink href="/payments" label="Pembayaran" id={d.related.paymentId} /> : null}
                {d.related.shipmentId ? <RelatedLink href={`/shipping/${d.related.shipmentId}`} label="Pengiriman" id={d.related.shipmentId} /> : null}
                {d.related.userId ? <RelatedLink href={`/users/${d.related.userId}`} label="Pelanggan" id={d.related.userId} /> : null}
              </div>
            </Card>
          ) : null}
          ...
          <Card>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-2 text-xs">
                <button onClick={() => setOpen(expandState(segments.length, true))} className="rounded-lg px-3 py-1.5 font-medium text-gray-600 ring-1 ring-gray-200 hover:bg-gray-50">Buka semua</button>
                <button onClick={() => setOpen(expandState(segments.length, false))} className="rounded-lg px-3 py-1.5 font-medium text-gray-600 ring-1 ring-gray-200 hover:bg-gray-50">Tutup semua</button>
              </div>
            </div>
            {segments.length === 0 ? (
              <p className="p-8 text-center text-sm text-gray-500">Tidak ada riwayat untuk request ini.</p>
            ) : (
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-5 text-sm">
          <section>
            ...
            <dl className="space-y-1.5">
              ...
              <Field label="Waktu" value={dt(event.time)} />
              ...
              <Field label="Modul" value={event.module} />
              <Field label="Aksi" value={event.action} />
              <Field label="Durasi" value={event.durationMs != null ? `${event.durationMs}ms` : '—'} />
              <Field label="Status" value={event.statusCode ?? '—'} />
            </dl>
          </section>

          <section>
            <p className="mb-2 text-xs font-semibold uppercase text-gray-400">Pesan</p>
          ...
          {stack ? (
            <section>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button onClick={() => copy(stack)} className="inline-flex items-center gap-1 text-xs font-medium text-[#465fff]"><Copy className="h-3.5 w-3.5" /> Salin Stack</button>
              </div>
