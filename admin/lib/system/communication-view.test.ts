...
test('history badges: success/failed/queued outcome + manual/auto origin', () => {
  assert.deepEqual(historyBadges({ status: 'SENT', attempts: 1, isManual: false, resendAt: null }).map((b) => b.label), ['Berhasil', 'Otomatis']);
  assert.deepEqual(historyBadges({ status: 'FAILED', attempts: 1, isManual: true, resendAt: null }).map((b) => b.label), ['Gagal', 'Manual']);
  assert.deepEqual(historyBadges({ status: 'PENDING', attempts: 0, isManual: false, resendAt: null }).map((b) => b.label), ['Diantrekan', 'Otomatis']);
});
...
test('history badges: retry and resend markers are additive', () => {
  ...
  assert.deepEqual(labels, ['Berhasil', 'Coba ulang ×2', 'Kirim ulang', 'Otomatis']);
});
...
test('delivery timeline: happy path ends with Provider Accepted then Delivered', () => {
  ...
  assert.deepEqual(steps.map((s) => s.label), ['Diantrekan', 'Mengirim', 'Diterima penyedia', 'Terkirim']);
...
test('delivery timeline: pending retry is scheduled with a timestamp', () => {
  ...
  assert.equal(steps.at(-1)?.label, 'Coba ulang #1');
...
test('conversation groups: order preferred, then payment/shipment, else General; order preserved', () => {
  ...
  assert.deepEqual(groups.map((g) => [g.kind, g.label, g.items.map((i) => i.id)]), [
    ['order', 'Pesanan BMS-1', ['a', 'c']],
    ['payment', 'Pembayaran p1', ['b']],
    ['general', 'Umum', ['d']],
  ]);
});

test('template labels: journey names match the spec example', () => {
  assert.equal(templateLabel({ template: 'order.transfer', stage: null, statusLabel: null, isManual: false }), 'Pesanan Dibuat — Menunggu Pembayaran');
  assert.equal(templateLabel({ template: 'payment.reminder', stage: 'first', statusLabel: null, isManual: false }), 'Pengingat Pembayaran (24 jam)');
  assert.equal(templateLabel({ template: 'payment.reminder', stage: 'second', statusLabel: null, isManual: false }), 'Pengingat Pembayaran (48 jam)');
  assert.equal(templateLabel({ template: 'payment.receipt_uploaded', stage: null, statusLabel: null, isManual: false }), 'Bukti Pembayaran Diunggah');
  assert.equal(templateLabel({ template: 'payment.approved', stage: null, statusLabel: null, isManual: false }), 'Pembayaran Terverifikasi');
  assert.equal(templateLabel({ template: 'order.shipped', stage: null, statusLabel: null, isManual: false }), 'Dikirim');
  assert.equal(templateLabel({ template: 'order.delivered', stage: null, statusLabel: null, isManual: false }), 'Diterima');
  assert.equal(templateLabel({ template: 'shipment.status', stage: null, statusLabel: 'sedang diantar', isManual: false }), 'Pengiriman: sedang diantar');
  assert.equal(templateLabel({ template: 'manual.custom', stage: null, statusLabel: null, isManual: true }), 'Pesan Manual');
